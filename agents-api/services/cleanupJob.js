...
const runCleanupJob = () => {
    ...
    // 3. Reset de Créditos à Meia-Noite (Plano Free, exceto super-admin)
    const today = new Date().toISOString().split('T')[0]; // YYYY-MM-DD
    const usersToReset = db.prepare("SELECT * FROM users WHERE plan = 'free' AND (role IS NULL OR role = 'user') AND last_reset < ?").all(today);
